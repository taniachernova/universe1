...
export const getProductsByTag = (products, tag, limit) => {
    const filteredProducts = products.map(product => ({
        ...product,
        tag: product.types.find(item => item === tag)
 ...
 * @returns {*[]}
 */
export const getRelatedProducts = (categories, types, products, limit) => {
    const identityMap = {};
    ...
    const addTagsPoints = (product, types) => {
        const point = 1;
        const id = getID(product);
        const prodTag = product.types.map((tag) => tag);
        const compareToTag = types.map((tag) => tag);
        prodTag.forEach((tag) => {
    ...
    for (const product of products) {
        ...
        if (types.length) addTagsPoints(product, types);
    }
...
export const getProductsUniqueTags = (products, limit) => {
    const types = products.map(product => product.types);
    const filteredTags = [...new Set(flatDeep(types))];
